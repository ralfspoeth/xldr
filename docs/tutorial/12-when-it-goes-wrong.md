...
A schema is named for the version of the format, so `mapping-spec-1.0` is the current one and stays current for
every `1.x` release that adds nothing. From 1.0 a later schema never refuses what an earlier one accepted, so
pinning to this pair is safe for as long as you are on a 1.x. The full list is on the
[schema page](https://ralfspoeth.github.io/xldr/).
