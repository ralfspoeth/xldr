...
[← your first spec](02-first-spec.md) · [index](README.md) · [next: constants →](04-constants.md)
...
The last row is the only real difference between the two formats, and page 5 comes back to it.
...
the spec needs to tell those two apart it uses two names rather than one - you will meet this on page 9, where a
field either names a column or counts one, and the two are `selector` and `nth` rather than one attribute read two
ways. Keeping the formats able to say the same things is what drives that.
